...
// THEN I am presented with a 5-day forecast that displays the date, an icon representation of weather conditions, the temperature, and the humidity - NEED CARD AND APPENDED INFO
...
$(".button").click(function(){
    ...
    $.ajax({
        ...
        for (var i = 0; i < onecall.daily.length; i++){
            ...
            $(figure).attr("class", "card");

            var dailyTempK = onecall.daily[i].temp.day;
            var dailyTempF = $("<p>").html(Math.round((dailyTempK*(9/5))-459.67));

            var dailyHumidity = $("<p>").html(onecall.daily[i].humidity);
